...
import { GLTFLoader, RoomEnvironment, SkeletonUtils } from 'three-stdlib'
import { butterflyConfig, type ButterflyConfig } from './config'
import { FlightModel } from './FlightModel'
import { WingController } from './WingController'

// The Electron build imported this from a shared settings module; a local
// ceiling is enough here (this web embed only ever asks for a handful).
...
export class ButterflyController {
  ...
  constructor(canvas: HTMLCanvasElement, config: ButterflyConfig = butterflyConfig) {
    ...
    // three-stdlib exposes RoomEnvironment as a factory function (not a class).
    this.envTexture = pmrem.fromScene(RoomEnvironment(), 0.04).texture
    this.scene.environment = this.envTexture
    pmrem.dispose()

    this.resize()
    // Size to the canvas's own box (it lives inside the monitor screen), not
    // the window — the original ran full-screen.
